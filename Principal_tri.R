...
base_crimes$ano_tri <- paste(base_crimes$ano_tri, 'º Trimestre', sep='')
...
base_letalidade <- left_join(base_crimes, base_corregedoria, by=c("ano_tri", "cod_reg")) %>% 
  select(reg_ano, ano, cod_reg, ano_tri, let_ser, let_fol, lesao_morte, hd_vitima, lat_vitima) %>% 
...
base_letalidade_long <- base_letalidade %>% 
  pivot_longer(!ano_tri, names_to = "morte", values_to = "count")

base_crimes_long <- base_crimes %>% 
  select(reg_ano, ano, cod_reg, ano_tri, roubo_outros, roubo_veic, lat_ocorr, hd_ocorr, 
...
base_crimes_long <- base_crimes_long %>% 
  pivot_longer(!ano_tri, names_to = "crime", values_to = "count")

base_corregedoria_long <- base_corregedoria %>% 
  filter(cod_ano >(ano_referencia-2)) %>% 
  select(cod_reg, ano_tri, let_ser, let_fol, mort_ser, mort_fol) %>% 
  filter(cod_reg!="30") %>%
  group_by(ano_tri) %>% 
  summarise(let_ser = sum(let_ser),
            let_fol = sum(let_fol),
...
base_corregedoria_long <- base_corregedoria_long%>% 
  pivot_longer(!ano_tri, names_to = "let_vit", values_to = "count")
